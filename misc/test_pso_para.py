...
def worker(index_and_values, template, ref_data, e_offset):
    index, para_values = index_and_values
    score = obj_function(para_values, template, ref_data, e_offset, str(index)+'-lmp.in')
    return score
...
def obj_function_par(para_values, template, ref_data, e_offset, ncpu):
    ...
    if ncpu == 1:
        for i, para_value in enumerate(para_values):
            score = obj_function(para_value, template, ref_data, e_offset, path='lmp.in')
            print(i, score)
            scores.append(score)
    else:
        if not hasattr(obj_function_par, 'pool'):
            ...
            #)
            obj_function_par.pool = mp.Pool(processes=ncpu)
            print(f"Pool initialized, {time()-t0}")

        for i, para_value in enumerate(para_values):
            path = f'{i}-lmp.in'
            write_lmp(para_value, template, path)
        index_and_values = list(enumerate(para_values))
        partial_worker = partial(worker, template=template, ref_data=ref_data, e_offset=e_offset)
        scores = obj_function_par.pool.map(partial_worker, index_and_values)
...
def write_lmp(para_value, template, path):
    strs = """
clear 
...
@timeit
def obj_function(para_value, template, ref_data, e_offset, path, obj='R2', write=False):
    """
    ...
    f_coef = 1
    s_coef = 1
    if write: write_lmp(para_value, template, path)
